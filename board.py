...
class Board:
    ...
    def evaluate_line_with_defense(self, row, col, direction, player_symbol, opponent_symbol):
    ...
    def check_winner(self, player_symbol):
        ...
        for row in range(self.rows):
            for col in range(self.cols - self.win + 1):
                if all(self.board[row][col + i] == player_symbol for i in range(self.win)):
                    return True

    
        for col in range(self.cols):
            for row in range(self.rows - self.win + 1):
                if all(self.board[row + i][col] == player_symbol for i in range(self.win)):
                    return True

        for row in range(self.rows - self.win + 1):
            for col in range(self.cols - self.win + 1):
                if all(self.board[row + i][col + i] == player_symbol for i in range(self.win)):
                    return True
