...
class Graphics:
    def __init__(self, board, background_color=(30, 150, 140), line_color=(23, 140, 132)):
        self.board = board
    ...
    def calculate_cell_size(self):
        ...
        min_size = 40
        max_size = 700
        return min(max(max_board_pixel_size // max(self.board.rows, self.board.cols), min_size), max_size)
    ...
    def display_grid_lines(self):
        ...
        for col in range(1, self.board.cols):
            pygame.draw.line(self.screen, self.line_color, (col * self.cell_size, 0), 
                             (col * self.cell_size, self.height), self.line_width)
        for row in range(1, self.board.rows):
            pygame.draw.line(self.screen, self.line_color, (0, row * self.cell_size), 
                             (self.width, row * self.cell_size), self.line_width)
    ...
    def draw_cross(self, row, col):
        ...
        end_desc = (col * self.cell_size + self.cell_size - offset, row * self.cell_size + self.cell_size - offset)
        pygame.draw.line(self.screen, (66, 66, 66), start_desc, end_desc, self.line_width * 2)
        start_asc = (col * self.cell_size + offset, row * self.cell_size + self.cell_size - offset)
        end_asc = (col * self.cell_size + self.cell_size - offset, row * self.cell_size + offset)
        pygame.draw.line(self.screen, (66, 66, 66), start_asc, end_asc, self.line_width * 2)
    ...
    def draw_circle(self, row, col):
        ...
        pygame.draw.circle(self.screen, (66, 66, 66), center, radius, self.line_width * 2)
    ...
    def choose_symbol(self):
        ...
        x_button_rect = pygame.Rect(self.width // 2 - 150, self.height // 2, button_width, button_height)
        o_button_rect = pygame.Rect(self.width // 2 + 50, self.height // 2, button_width, button_height)

        font = pygame.font.Font(None, 60)
        self.draw_board()

        # Render welcome message and buttons
    ...
    def wait_for_restart_or_quit(self):
        ...
        play_again_button = pygame.Rect(self.width // 2 - 100, self.height // 2 + 50, 200, 40)
        ...
        self.screen.blit(play_again_text, (play_again_button.x + 50, play_again_button.y + 5))
